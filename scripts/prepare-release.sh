...
if [ ! -f ~/.tauri/rheo.key ]; then
  echo "❌ Private key not found at ~/.tauri/rheo.key"
  echo "Run: cd tauri && bun tauri signer generate -w ~/.tauri/rheo.key"
  exit 1
fi

if [ ! -f ~/.tauri/rheo.key.pub ]; then
  echo "❌ Public key not found at ~/.tauri/rheo.key.pub"
  exit 1
fi
...
export TAURI_SIGNING_PRIVATE_KEY="$(cat ~/.tauri/rheo.key)"
