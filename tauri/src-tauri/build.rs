...
fn main() {
    ...
    #[cfg(target_os = "macos")]
    {
        // rheo.icon is in tauri/assets/rheo.icon (one level up from src-tauri)
        let icon_source = format!("{}/../assets/rheo.icon", project_root);
        ...
        if std::path::Path::new(&icon_source).exists() {
            ...
            // Generate rheo.icns from the source PNG via sips + iconutil
            let icns_path = format!("{}/rheo.icns", gen_dir);
            if !std::path::Path::new(&icns_path).exists() {
                let source_png = format!("{}/Assets/Rheo.png", icon_source);
                if std::path::Path::new(&source_png).exists() {
                    let iconset_dir = format!("{}/rheo.iconset", gen_dir);
    ...
    {
        let required = ["Assets.car", "rheo.icns", "partial.plist"];
