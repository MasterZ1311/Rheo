...
#[cfg(target_os = "windows")]
pub fn send_submit(key: &str) -> Result<(), String> {
    use windows::Win32::UI::Input::KeyboardAndMouse::{
        SendInput, INPUT, VK_CONTROL, VK_RETURN,
    };

    let events = match key {
        "ctrl_enter" | "cmd_enter" => vec![
            win::make_key(VK_CONTROL, false),
            win::make_key(VK_RETURN, false),
            win::make_key(VK_RETURN, true),
            win::make_key(VK_CONTROL, true),
        ],
        _ => vec![
            win::make_key(VK_RETURN, false),
    ...
    unsafe {
        ...
        if sent as usize != events.len() {
            return Err(format!(
                "SendInput delivered {} of {} submit events",
                sent,
                events.len()
            ));
        }
...
pub fn send_submit(key: &str) -> Result<(), String> {
    ...
    const KEYCODE_RETURN: CGKeyCode = 0x24; // kVK_Return on macOS
    ...
    unsafe {
        ...
        let events = match key {
            "cmd_enter" | "ctrl_enter" => vec![
                (KEYCODE_LEFT_CMD, true, K_CG_EVENT_FLAG_MASK_COMMAND),
                (KEYCODE_RETURN, true, K_CG_EVENT_FLAG_MASK_COMMAND),
                (KEYCODE_RETURN, false, K_CG_EVENT_FLAG_MASK_COMMAND),
                (KEYCODE_LEFT_CMD, false, 0),
            ],
            _ => vec![
                (KEYCODE_RETURN, true, 0),
                (KEYCODE_RETURN, false, 0),
            ],
        };
        ...
        for (k, down, flags) in events {
            let event = CGEventCreateKeyboardEvent(source, k, down);
            if event.is_null() {
                return Err(format!(
                    "CGEventCreateKeyboardEvent(key={}, down={}) returned null",
                    k, down
...
#[cfg(not(any(target_os = "macos", target_os = "windows")))]
pub fn send_submit(_key: &str) -> Result<(), String> {
    Err("synthetic submit is not yet implemented on this platform".into())
}
