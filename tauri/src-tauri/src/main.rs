...
/// Find a rheo-server process listening on a given port (Windows only).
///
...
/// Check if a Rheo server is responding on the given port.
///
/// Sends an HTTP GET to `/health` and returns `true` only if the response
/// is valid JSON matching the Rheo `HealthResponse` schema — specifically
        ...
        .build()
    {
        Ok(client) => match client.get(&url).send() {
            Ok(resp) => {
                ...
                // Parse as JSON and validate Rheo-specific fields
...
async fn start_server(
    ...
    // Check if a rheo server is already running on our port (from previous session with keep_running=true,
            ...
            .output()
        {
            ...
            for line in output_str.lines().skip(1) {
                ...
                if parts.len() >= 2 {
                    let command = parts[0];
                    let pid_str = parts[1];
                    if command.contains("rheo") {
                        if let Ok(pid) = pid_str.parse::<u32>() {
                            println!("Found existing rheo-server on port {} (PID: {}), reusing it", SERVER_PORT, pid);
                            // Store the PID so we can kill it on exit if needed
                            *state.server_pid.lock().unwrap() = Some(pid);
                            return Ok(format!("http://127.0.0.1:{}", SERVER_PORT));
                        }
                    } else {
                        // Process name doesn't contain "rheo" — could be an external
                        // Python/uvicorn/Docker server. Verify via HTTP health check.
                        println!("Port {} in use by '{}' (PID: {}), checking if it's a Rheo server...", SERVER_PORT, command, pid_str);
                        if check_health(SERVER_PORT) {
                            println!("Health check passed — reusing external server on port {}", SERVER_PORT);
                            return Ok(format!("http://127.0.0.1:{}", SERVER_PORT));
                        }
                        println!("Health check failed — port is occupied by a non-Rheo process");
                        return Err(format!(
                            "Port {} is already in use by another application ({}). \
                             Close it or change the Rheo server port.",
                            SERVER_PORT, command
                        ));
                    }
                }
    ...
    #[cfg(windows)]
    {
        ...
        if TcpStream::connect_timeout(
            ...
            // Port is in use — check if it's a rheo process by name first
            if let Some(pid) = find_rheo_pid_on_port(SERVER_PORT) {
                println!("Found existing rheo-server on port {} (PID: {}), reusing it", SERVER_PORT, pid);
                *state.server_pid.lock().unwrap() = Some(pid);
                return Ok(format!("http://127.0.0.1:{}", SERVER_PORT));
            }
            ...
            println!("Port {} in use by unknown process, checking if it's a Rheo server...", SERVER_PORT);
            if check_health(SERVER_PORT) {
                println!("Health check passed — reusing external server on port {}", SERVER_PORT);
                return Ok(format!("http://127.0.0.1:{}", SERVER_PORT));
            }
            return Err(format!(
                "Port {} is already in use by another application. \
                 Close the other application or change the Rheo port.",
                SERVER_PORT
            ));
        }
    }

    // Kill any orphaned rheo-server from previous session on legacy port 8000
                ...
                if parts.len() >= 2 {
                    ...
                    if command.contains("rheo") {
                        if let Ok(pid) = pid_str.parse::<i32>() {
                            println!("Found orphaned rheo-server on legacy port {} (PID: {}, CMD: {}), killing it...", LEGACY_PORT, pid, command);
                        ...
                        println!("Legacy port {} is in use by non-rheo process: {} (PID: {}), not killing", LEGACY_PORT, command, pid_str);
                    }
                }
            }
        }
    ...
    #[cfg(windows)]
    {
        ...
        if TcpStream::connect_timeout(
            ...
            if let Some(pid) = find_rheo_pid_on_port(LEGACY_PORT) {
                println!("Found orphaned rheo-server on legacy port {} (PID: {}), killing it...", LEGACY_PORT, pid);
    ...
    println!("=================================================================");
    println!("Starting rheo-server sidecar");
    ...
    let sidecar_result = app.shell().sidecar("rheo-server");
    ...
    let spawn_result = if backend_override.as_deref() != Some("cpu") {
        ...
        if let Some(ref rocm_path) = rocm_binary {
            ...
            if let Some(ref dir) = effective_models_dir { cmd = cmd.env("RHEO_MODELS_DIR", dir); }
            match cmd.spawn() {
        ...
        if let Some(result) = gpu_spawn {
            ...
            if let Some(ref dir) = effective_models_dir { sidecar = sidecar.env("RHEO_MODELS_DIR", dir); }
        ...
        if let Some(ref dir) = effective_models_dir {
            sidecar = sidecar.env("RHEO_MODELS_DIR", dir);
        }
        sidecar.spawn()
    };
        ...
        match tokio::time::timeout(tokio::time::Duration::from_millis(100), rx.recv()).await {
            ...
            Ok(None) => {
                ...
                #[cfg(not(debug_assertions))]
                {
                    ...
                    eprintln!("Check Console.app logs for more details (search for 'rheo')");
...
/// Identifier of the Rheo app itself — used to short-circuit auto-paste
/// when the user fires a chord while focus was inside one of our own
/// windows. Paste into Rheo-internal targets is step 6 territory and
...
#[cfg(target_os = "macos")]
const RHEO_BUNDLE_ID: &str = "sh.rheo.app";
#[cfg(target_os = "windows")]
const RHEO_BUNDLE_ID: &str = "rheo.exe";
#[cfg(not(any(target_os = "macos", target_os = "windows")))]
const RHEO_BUNDLE_ID: &str = "rheo";
...
///
/// On macOS this is the call that triggers the "Rheo would like to receive
...
/// - `focus.bundle_id` is Rheo itself — step 6 will inject directly
...
async fn paste_final_text(
    ...
    if focus.bundle_id.as_deref() == Some(RHEO_BUNDLE_ID) {
        return Ok(false);
    }
    if !accessibility::is_trusted() {
        return Err(
            "Accessibility permission required for auto-paste. Open System Settings → Privacy & Security → Accessibility and enable Rheo."
                .into(),
        );
    }

    let settle_ms = paste_delay_ms.unwrap_or(POST_ACTIVATE_SETTLE_MS);
    ...
    if safe_to_restore {
        ...
        eprintln!(
            "[rheo] clipboard mutated during paste window — skipping restore to preserve newer content"
        );
    }

    paste_result?;

    if auto_submit.unwrap_or(false) {
        tokio::time::sleep(std::time::Duration::from_millis(50)).await;
        let submit_key = auto_submit_key.as_deref().unwrap_or("enter");
        if let Err(e) = synthetic_keys::send_submit(submit_key) {
            eprintln!("[rheo] auto-submit failed: {e}");
        }
    }
...
async fn debug_focus_roundtrip(
    ...
    if !accessibility::is_trusted() {
        return Err(
            "Accessibility permission not granted. Open System Settings → Privacy & Security → Accessibility and enable Rheo."
                .into(),
        );
    }
...
async fn debug_paste_text(
    ...
    if !accessibility::is_trusted() {
        return Err(
            "Accessibility permission not granted. Open System Settings → Privacy & Security → Accessibility and enable Rheo, then try again."
                .into(),
        );
    }
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            #[cfg(desktop)]
            {
                ...
                // steals focus to the Rheo app when the user clicks
                ...
                // Agent-initiated speech (rheo.speak over MCP or POST /speak)
