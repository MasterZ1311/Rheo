"""Rheo Cloud device login routes.
...
async def start_cloud_login(request: Request):
    """Initiate browser-based device authorization with Rheo Cloud."""
...
async def cloud_callback(
    ...
    """OAuth callback handler exchanging the one-time code for a device token."""
    ...
    sub = (
        "Rheo is now linked to your account. You can close this tab and return to the app."
        if ok
        else message
    )
    html = f"""<!doctype html>
...
async def cloud_status(db: Session = Depends(get_db)):
    """Check whether this instance is currently paired with Rheo Cloud."""
...
async def cloud_disconnect(db: Session = Depends(get_db)):
    """Unlink this instance from Rheo Cloud and discard the stored token."""
