...
column is the same value the MCP client sends in ``X-Rheo-Client-Id``
(or the stdio shim pulls from ``RHEO_CLIENT_ID``).
"""
...
async def list_mcp_bindings(db: Session = Depends(get_db)):
    """List all registered MCP client bindings and their default voices."""
    rows = (
...
async def delete_mcp_binding(
    ...
    """Delete an MCP client binding by client ID."""
    row = (
        db.query(MCPClientBinding)
