...
MAX_CAPTURE_SIZE = 100 * 1024 * 1024  # 100 MB maximum upload limit
...
async def create_capture_endpoint(
    ...
    while chunk := await file.read(UPLOAD_CHUNK_SIZE):
        ...
        if total_size > MAX_CAPTURE_SIZE:
            raise HTTPException(
                status_code=413,
                detail=f"Uploaded file exceeds maximum size limit of {MAX_CAPTURE_SIZE // (1024 * 1024)} MB",
            )
        chunks.append(chunk)
...
async def list_captures_endpoint(
    ...
    """List paginated voice captures, ordered by most recent first."""
...
async def get_capture_endpoint(capture_id: str, db: Session = Depends(get_db)):
    """Retrieve metadata and transcripts for a specific voice capture."""
...
async def delete_capture_endpoint(capture_id: str, db: Session = Depends(get_db)):
    """Delete a voice capture record and its underlying audio file."""
...
async def refine_capture_endpoint(
    ...
    """Re-run LLM refinement on an existing capture transcript with updated flags."""
    saved = settings_service.get_capture_settings(db)
...
async def retranscribe_capture_endpoint(
    ...
    """Re-run Whisper speech-to-text on an existing capture audio file."""
    saved = settings_service.get_capture_settings(db)
