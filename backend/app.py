...
def create_app() -> FastAPI:
    ...
    @asynccontextmanager
    async def rheo_lifespan(app: FastAPI):
    ...
    # compose_lifespan enters factories in order (Rheo startup →
    ...
    lifespan = compose_lifespan(rheo_lifespan, mcp_app.router.lifespan_context)

    application = FastAPI(
        title="Rheo API",
        description="Rheo Sovereign AI Voice Studio & Neural Speech Synthesis Engine — Architected by masterz1311",
        version=__version__,
        lifespan=lifespan,
    )
    ...
    async def security_headers_middleware(request: Request, call_next):
        ...
        response.headers["X-License"] = "Apache-2.0"
        return response
    ...
    async def csrf_origin_guard(request: Request, call_next):
        if request.method in ("POST", "PUT", "DELETE", "PATCH"):
            ...
            if origin:
                ...
                env_origins = {
                    o.strip()
                    for o in os.environ.get("RHEO_CORS_ORIGINS", "").split(",")
                    if o.strip()
                }
                allowed = default_origins | env_origins
                is_local = origin.startswith("http://localhost:") or origin.startswith("http://127.0.0.1:")
                if origin not in allowed and not is_local:
                    logger.warning("Blocked potential CSRF request from origin: %s to %s", origin, request.url.path)
                    return JSONResponse(
                        status_code=403,
    ...
    _rate_limit_paths = {"/generate", "/transcribe", "/llm/generate"}
    _rate_limit_window = 60.0

    @application.middleware("http")
    async def rate_limiting_middleware(request: Request, call_next):
        if request.url.path in _rate_limit_paths and request.method == "POST":
            limit_str = os.environ.get("RHEO_RATE_LIMIT_PER_MINUTE", "60")
            limit = int(limit_str)
            if limit > 0:
                client_ip = request.client.host if request.client else "unknown"
                client_id = (
                    request.headers.get("X-Rheo-Client-Id")
                    or client_ip
                )
                now = time.time()
                async with _rate_limit_lock:
                    timestamps = _rate_limit_buckets.setdefault(client_id, [])
    ...
    allowed_hosts = ["localhost", "127.0.0.1", "[::1]", "::1", "tauri.localhost", "*.localhost", "testserver"]
    env_hosts = [
        h.strip()
        for h in os.environ.get("RHEO_ALLOWED_HOSTS", "").split(",")
        if h.strip()
    ]
    if env_hosts:
...
def _configure_cors(application: FastAPI) -> None:
    ...
    env_origins = os.environ.get("RHEO_CORS_ORIGINS", "")
...
async def _run_startup(application: FastAPI) -> None:
    ...
    logger.info("Rheo v%s starting up (by masterz1311)", __version__)
    logger.info(
        "Python %s on %s %s (%s)",
        sys.version.split()[0],
        platform.system(),
...
async def _run_shutdown() -> None:
    ...
    logger.info("Rheo server shutting down...")
    try:
