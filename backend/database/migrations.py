"""Column-level migrations for the rheo SQLite database.

Why not Alembic?  rheo is a single-user desktop app shipping as a
...
def _migrate_capture_settings(engine, inspector, tables: set[str]) -> None:
    ...
    if "audio_feedback" not in columns:
        _add_column(
            engine,
            "capture_settings",
            "audio_feedback BOOLEAN NOT NULL DEFAULT 0",
            "audio_feedback",
        )
    if "audio_feedback_volume" not in columns:
        _add_column(
            engine,
            "capture_settings",
            "audio_feedback_volume FLOAT NOT NULL DEFAULT 0.7",
            "audio_feedback_volume",
        )
    if "sound_theme" not in columns:
        _add_column(
            engine,
            "capture_settings",
            "sound_theme VARCHAR NOT NULL DEFAULT 'pop'",
            "sound_theme",
        )
    if "custom_words" not in columns:
        _add_column(
            engine,
            "capture_settings",
    ...
    if "word_correction_threshold" not in columns:
        _add_column(
            engine,
            "capture_settings",
            "word_correction_threshold FLOAT NOT NULL DEFAULT 0.8",
            "word_correction_threshold",
        )
    if "auto_submit" not in columns:
        _add_column(
            engine,
            "capture_settings",
    ...
    if "auto_submit_key" not in columns:
        _add_column(
            engine,
            "capture_settings",
            "auto_submit_key VARCHAR NOT NULL DEFAULT 'enter'",
            "auto_submit_key",
        )
    if "append_trailing_space" not in columns:
        _add_column(
            engine,
            "capture_settings",
            "append_trailing_space BOOLEAN NOT NULL DEFAULT 0",
            "append_trailing_space",
        )
    if "recording_retention_period" not in columns:
        _add_column(
            engine,
            "capture_settings",
            "recording_retention_period VARCHAR NOT NULL DEFAULT 'preserve_limit'",
    ...
    if "translate_to_english" not in columns:
        _add_column(
            engine,
            "capture_settings",
            "translate_to_english BOOLEAN NOT NULL DEFAULT 0",
            "translate_to_english",
        )
    if "model_unload_timeout_minutes" not in columns:
        _add_column(
            engine,
            "capture_settings",
            "model_unload_timeout_minutes INTEGER NOT NULL DEFAULT 5",
            "model_unload_timeout_minutes",
        )
    if "remove_filler_words" not in columns:
        _add_column(
            engine,
            "capture_settings",
            "remove_filler_words BOOLEAN NOT NULL DEFAULT 0",
            "remove_filler_words",
        )
    if "paste_delay_ms" not in columns:
        _add_column(
            engine,
            "capture_settings",
            "paste_delay_ms INTEGER NOT NULL DEFAULT 50",
            "paste_delay_ms",
        )
...
def _migrate_mcp_bindings(engine, inspector, tables: set[str]) -> None:
    ...
    to a boolean: when true, ``rheo.speak`` rewrites input through the
