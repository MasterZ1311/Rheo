"""ORM model definitions for the rheo SQLite database."""
...
class CaptureSettings(Base):
    ...
    # "Rheo would like to receive keystrokes from any application" dialog
    ...
    # Handy features: audio feedback cues (pop, marimba)
    audio_feedback = Column(Boolean, nullable=False, default=False)
    audio_feedback_volume = Column(Float, nullable=False, default=0.7)
    sound_theme = Column(String, nullable=False, default="pop")
    # Custom vocabulary dictionary & fuzzy matching correction threshold
    custom_words = Column(JSON, nullable=False, default=list)
    word_correction_threshold = Column(Float, nullable=False, default=0.8)
    # Auto-submit after transcription paste (enter, ctrl_enter, cmd_enter)
    auto_submit = Column(Boolean, nullable=False, default=False)
    auto_submit_key = Column(String, nullable=False, default="enter")
    # Append trailing space after paste
    append_trailing_space = Column(Boolean, nullable=False, default=False)
    # History retention policy and limits
    recording_retention_period = Column(String, nullable=False, default="preserve_limit")
    history_limit = Column(Integer, nullable=False, default=200)
    # Whisper task=translate directly to English
    translate_to_english = Column(Boolean, nullable=False, default=False)
    # Idle STT/TTS model unload timeout in minutes (0 to disable)
    model_unload_timeout_minutes = Column(Integer, nullable=False, default=5)
    # Fast rule-based filler word stripper ("um", "uh", etc.)
    remove_filler_words = Column(Boolean, nullable=False, default=False)
    # Pre-paste delay in milliseconds
    paste_delay_ms = Column(Integer, nullable=False, default=50)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class CloudSettings(Base):
    """Singleton row holding the link to a Rheo Cloud account.
    ...
    api.rheo.sh — auth only, never an encryption key (E2E key material lives
...
class MCPClientBinding(Base):
    ...
    itself via the ``X-Rheo-Client-Id`` HTTP header; direct-HTTP
    clients set it in their MCP config's ``headers`` block, the stdio
    shim forwards it from the ``RHEO_CLIENT_ID`` env var.
    """
    ...
    # When true, rheo.speak routes through the profile's personality LLM
