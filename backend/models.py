...
class CaptureSettingsResponse(BaseModel):
    ...
    audio_feedback_volume: float = Field(default=0.7, ge=0.0, le=1.0)
    sound_theme: str = Field(default="pop", pattern="^(pop|marimba)$")
    custom_words: List[str] = Field(default_factory=list)
    word_correction_threshold: float = Field(default=0.8, ge=0.0, le=1.0)
    auto_submit: bool = False
    auto_submit_key: str = Field(default="enter", pattern="^(enter|ctrl_enter|cmd_enter)$")
    append_trailing_space: bool = False
    recording_retention_period: str = Field(
        default="preserve_limit", pattern="^(never|preserve_limit|3_days|2_weeks|3_months)$"
    )
    history_limit: int = Field(default=200, ge=10, le=10000)
    translate_to_english: bool = False
    model_unload_timeout_minutes: int = Field(default=5, ge=0, le=1440)
    remove_filler_words: bool = False
    paste_delay_ms: int = Field(default=50, ge=0, le=2000)
...
class CaptureSettingsUpdate(BaseModel):
    ...
    sound_theme: Optional[str] = Field(default=None, pattern="^(pop|marimba)$")
    custom_words: Optional[List[str]] = None
    word_correction_threshold: Optional[float] = Field(default=None, ge=0.0, le=1.0)
    auto_submit: Optional[bool] = None
    auto_submit_key: Optional[str] = Field(default=None, pattern="^(enter|ctrl_enter|cmd_enter)$")
    append_trailing_space: Optional[bool] = None
    recording_retention_period: Optional[str] = Field(
        default=None, pattern="^(never|preserve_limit|3_days|2_weeks|3_months)$"
    )
    history_limit: Optional[int] = Field(default=None, ge=10, le=10000)
    translate_to_english: Optional[bool] = None
    model_unload_timeout_minutes: Optional[int] = Field(default=None, ge=0, le=1440)
    remove_filler_words: Optional[bool] = None
    paste_delay_ms: Optional[int] = Field(default=None, ge=0, le=2000)
...
class MCPClientBindingResponse(BaseModel):
    ...
    use when a given client_id calls rheo.speak without args, plus an
...
class MCPClientBindingListResponse(BaseModel):
    """Response model wrapping a list of MCP client bindings."""

    items: List[MCPClientBindingResponse]


class SpeakRequest(BaseModel):
    """Body for POST /speak — non-MCP REST surface that mirrors rheo.speak."""
...
class CloudStatusResponse(BaseModel):
    """Current link between this device and a Rheo Cloud account."""
