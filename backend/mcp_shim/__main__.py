"""rheo-mcp — stdio ↔ Streamable-HTTP MCP proxy.
...
server's response back. The Rheo server does all the real work.

Environment variables:
  RHEO_PORT       Rheo server port (default 17493).
  RHEO_HOST       Host (default 127.0.0.1).
  RHEO_CLIENT_ID  Forwarded as X-Rheo-Client-Id on every request.
...
CLIENT_ID_HEADER = "X-Rheo-Client-Id"
...
def _err(msg: str) -> None:
    print(f"rheo-mcp: {msg}", file=sys.stderr, flush=True)


def _base_url() -> tuple[str, str]:
    host = os.environ.get("RHEO_HOST", "127.0.0.1")
    port = int(os.environ.get("RHEO_PORT", str(DEFAULT_PORT)))
            ...
            _write_stdout(
                {
                    ...
                    "error": {
                        "code": -32000,
                        "message": (
                            f"Rheo MCP proxy got HTTP {response.status_code}"
                        ),
                    },
                }
            )
...
async def _run() -> int:
    ...
    client_id = os.environ.get("RHEO_CLIENT_ID")
    if client_id:
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(300.0)) as client:
        if not await _wait_for_backend(client, health_url):
            _err(
                f"timed out waiting for Rheo at {health_url} — is the app open?"
            )
            return 2
...
def main() -> int:
    """Entrypoint for the MCP stdio-to-HTTP proxy shim."""
    try:
