"""
Configuration module for Rheo backend.
...
# Set RHEO_MODELS_DIR to an absolute path before starting the server.
# This sets HF_HUB_CACHE so all huggingface_hub downloads go to that path.
_custom_models_dir = os.environ.get("RHEO_MODELS_DIR")
...
# Rheo Cloud (backup & sync). Two hosts: the web app owns auth + device
# pairing (rheo.sh), the API owns sync + account endpoints (api.rheo.sh).
# Override both for local development, e.g.
# RHEO_CLOUD_URL=http://localhost:17592 RHEO_CLOUD_API_URL=http://localhost:17593
def get_cloud_web_url() -> str:
    """Base URL of the Rheo Cloud web app (auth + /connect + exchange)."""
    return (
        os.environ.get("RHEO_CLOUD_URL")
        or "https://rheo.sh"
    ).rstrip("/")


def get_cloud_api_url() -> str:
    """Base URL of the Rheo Cloud API (bearer-authenticated sync/account)."""
    return (
        os.environ.get("RHEO_CLOUD_API_URL")
        or "https://api.rheo.sh"
    ).rstrip("/")
