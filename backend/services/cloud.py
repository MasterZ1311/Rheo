"""
Rheo Cloud device login — the "Log in with browser" flow.
...
callback. We exchange that code (server-to-server, over TLS) for a ``rheo_…``
    ...
    try:
        async with httpx.AsyncClient(timeout=15.0) as client:
            ...
            if payload is None:
                ...
                return False, "Rheo Cloud returned an unexpected response."
            api_key = payload.get("key")
            device_name = payload.get("label")
            if not api_key:
                return False, "Rheo Cloud did not return a key."
        ...
        return False, "Could not reach Rheo Cloud. Check your connection and try again."

    _store_key(db, api_key=api_key, device_name=device_name, account_user_id=account_user_id)
    logger.info("connected to Rheo Cloud as device %r", device_name)
...
def get_status(db: Session) -> dict:
    ...
    # Prefix only: "rheo_" (5) + 8 chars, matching the cloud's key_prefix.
    key_prefix = row.api_key[:13] if row.api_key else None
    return {
        "connected": connected,
