...
Downloads two archives from GitHub Releases:
  1. Server core (rheo-server-rocm.tar.gz) — the exe + non-AMD deps,
...
def get_rocm_exe_name() -> str:
    ...
    if sys.platform == "win32":
        return "rheo-server-rocm.exe"
    return "rheo-server-rocm"
...
def is_rocm_active() -> bool:
    ...
    """
    return os.environ.get("RHEO_BACKEND_VARIANT") == "rocm"
...
async def _download_rocm_binary_locked(version: Optional[str] = None):
    ...
    server_archive = "rheo-server-rocm.tar.gz"
