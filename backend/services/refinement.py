...
def remove_filler_words_rule(text: str) -> str:
    """Deterministically strip common disfluency filler words ('um', 'uh', 'er', 'ah', 'hmm').

    Provides a zero-latency rule-based filler word stripper (mirroring Handy's filler
    word removal) that works instantly without requiring a local LLM inference pass.
    """
    if not text:
        return text

    pattern = re.compile(r"\b(um|uh|er|ah|hmm)\b[,]?\s*", flags=re.IGNORECASE)
    cleaned = pattern.sub("", text)

    # Clean up orphaned punctuation or excessive whitespace
    cleaned = re.sub(r"\s+([,.?!])", r"\1", cleaned)
    cleaned = re.sub(r"\s+", " ", cleaned).strip()

    # Capitalize the first letter if removing a sentence-initial filler lowercased the rest
    if cleaned and text and text[0].isupper() and not cleaned[0].isupper():
