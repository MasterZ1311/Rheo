...
"""
Entry point for PyInstaller-bundled Rheo server.
...
if "--version" in sys.argv:
    ...
    print(f"rheo-server {__version__}")
    sys.exit(0)
...
if re.search(r"rheo-server-rocm(\.exe)?$", _binary_name):
    os.environ["RHEO_BACKEND_VARIANT"] = "rocm"
elif re.search(r"rheo-server-cuda(\.exe)?$", _binary_name):
    os.environ["RHEO_BACKEND_VARIANT"] = "cuda"
else:
    os.environ.setdefault("RHEO_BACKEND_VARIANT", "cpu")
...
logger.info("=" * 60)
logger.info("rheo-server starting up...")
...
if __name__ == "__main__":
    try:
        parser = argparse.ArgumentParser(description="rheo backend server")
        parser.add_argument(
            "--host",
            type=str,
            default="127.0.0.1",
        ...
        logger.info(f"Backend variant: {os.environ.get('RHEO_BACKEND_VARIANT', 'cpu').upper()}")
