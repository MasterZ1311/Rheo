...
The server runs inside the same `uvicorn` process as the rest of Rheo
...
`rheo-mcp` binary ships inside the Rheo.app bundle:
...
| Name | Purpose |
|---|---|
| `rheo.speak`          | Speak text in a voice profile. Returns a generation id you can poll. |
| `rheo.transcribe`     | Whisper transcription of a base64 blob or an absolute local path. |
| `rheo.list_captures`  | Recent captures (dictation / recording / file) with transcripts. |
...
2. Per-client binding keyed by `X-Rheo-Client-Id`
...
Point it at the URL, hit "List tools," call `rheo.list_profiles`
first to confirm wiring, then `rheo.speak` for end-to-end.
...
curl -X POST http://127.0.0.1:17493/speak \
  ...
  -H 'X-Rheo-Client-Id: claude-code' \
