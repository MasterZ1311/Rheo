"""Rheo MCP tool implementations.

Thin wrappers over existing services/routes. Tools are registered with dotted
names (``rheo.speak`` etc.) so they look natural in agent logs —
...
def register_tools(mcp: FastMCP) -> None:
    """Attach all Rheo tools to the given FastMCP instance."""

    @mcp.tool(
        name="rheo.speak",
        description=(
            "Speak text in a Rheo voice profile. Returns a generation id "
            "the caller can poll at /generate/{id}/status. Audio plays on the "
            "user's speakers and is saved to the Captures / History tab."
        ),
    )
    async def rheo_speak(
        text: str,
    ...
    async def rheo_transcribe(
        ...
        # to loopback callers so a Rheo bound on 0.0.0.0 doesn't double
    ...
    @mcp.tool(
        name="rheo.list_profiles",
        description=(
            "List available voice profiles (both cloned voices and presets). "
            "Use the returned `name` with rheo.speak(profile=...)."
        ),
    )
    async def rheo_list_profiles() -> dict[str, Any]:
        db = next(get_db())
        try:
...
async def _transcribe_file(
    ...
    if (
        ...
        raise ValueError(
            ...
            "Rheo → Settings → Models to download it first."
        )
