...
MCP clients identify themselves via an ``X-Rheo-Client-Id`` HTTP header
(direct-HTTP clients set it in their MCP config; the stdio shim forwards it
from the ``RHEO_CLIENT_ID`` env var). Middleware copies the value into a
...
# host-filesystem access to loopback callers (see rheo.transcribe).
...
# Endpoints that consume X-Rheo-Client-Id for its MCP-semantic
...
#
# - /mcp — FastMCP tool calls (rheo.speak, rheo.transcribe, …)
...
#   false stamps.
# - /speak — REST mirror of rheo.speak for non-MCP agents (shell
...
class ClientIdMiddleware(BaseHTTPMiddleware):
    """Copy X-Rheo-Client-Id into a ContextVar and stamp last_seen_at
