...
directly via URL; older stdio-only clients use the ``rheo-mcp`` shim
...
def build_mcp_server() -> FastMCP:
    """Create the FastMCP instance with Rheo tools registered."""
    mcp = FastMCP(
        name="rheo",
        instructions=(
            "Rheo is a local voice I/O layer. Use `rheo.speak` to "
            "play text in a voice profile, `rheo.transcribe` for "
...
def compose_lifespan(*lifespans):
    ...
    Used by ``create_app`` to run the existing Rheo startup/shutdown
