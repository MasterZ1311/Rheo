...
try:
    ...
    _HAS_PYINSTALLER = True
except ImportError:
    from unittest.mock import MagicMock

    PyInstaller = MagicMock()
    PyInstaller.__main__ = MagicMock()
    _HAS_PYINSTALLER = False
...
def build_server(cuda=False, rocm=False):
    ...
    Args:
        cuda: If True, build with CUDA support and name the binary
              rheo-server-cuda instead of rheo-server.
        rocm: If True, build with ROCm support and name the binary
              rheo-server-rocm instead of rheo-server.
    """
    if cuda and rocm:
    ...
    if rocm:
        binary_name = "rheo-server-rocm"
    elif cuda:
        binary_name = "rheo-server-cuda"
    else:
        binary_name = "rheo-server"
    ...
    args.extend(
        [
            ...
            "--hidden-import",
            "loguru",
            # MCP server — Streamable-HTTP endpoint and the 4 rheo.* tools.
...
def build_shim():
    """Build the rheo-mcp stdio shim as a tiny standalone binary.

    This is the bridge for MCP clients that only speak stdio — it proxies
    JSON-RPC to the main rheo-server's /mcp endpoint. Keep it small: no
    ...
    os.chdir(backend_dir)
    PyInstaller.__main__.run(args)
    logger.info("Shim built: %s", backend_dir / "dist" / "rheo-mcp")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Build rheo binaries")
    parser.add_argument(
        "--cuda",
        action="store_true",
        help="Build CUDA-enabled binary (rheo-server-cuda)",
    )
    parser.add_argument(
        "--rocm",
        action="store_true",
        help="Build ROCm-enabled binary (rheo-server-rocm) for AMD GPUs",
    )
    parser.add_argument(
        "--shim",
        action="store_true",
        help="Build the rheo-mcp stdio shim binary instead of the server",
    )
