...
def fake_tar_gz():
    ...
    with tarfile.open(fileobj=buf, mode="w:gz") as tar:
        ...
        info = tarfile.TarInfo(name="rheo-server-rocm.exe")
        info.size = len(data)
...
async def test_download_rocm_binary_progress_reporting(mock_backends_dir, fake_tar_gz, fake_sha256):
    ...
    assert (rocm_dir / "rheo-server-rocm.exe").exists()
...
async def test_is_rocm_active(mock_backends_dir, monkeypatch):
    monkeypatch.setenv("RHEO_BACKEND_VARIANT", "rocm")
    assert rocm.is_rocm_active() is True

    monkeypatch.setenv("RHEO_BACKEND_VARIANT", "cpu")
    assert rocm.is_rocm_active() is False

    monkeypatch.delenv("RHEO_BACKEND_VARIANT", raising=False)
    assert rocm.is_rocm_active() is False
