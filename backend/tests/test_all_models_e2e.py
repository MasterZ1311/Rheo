...
def find_binary() -> Optional[Path]:
    ...
    candidates = [
        DIST_DIR / "rheo-server-cuda" / f"rheo-server-cuda{exe}",
        DIST_DIR / f"rheo-server{exe}",
    ]
    for c in candidates:
...
def write_reports(
    ...
    lines = [
        f"# Rheo E2E — {plat} — {started_at.strftime('%Y-%m-%d %H:%M UTC')}",
        "",
        f"Binary: `{binary}`  ",
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Rheo E2E model generation test")
    p.add_argument("--binary", type=Path, help="Path to rheo-server binary (overrides auto-detect)")
...
def main() -> int:
    ...
    # Tempdir + log path
    data_dir = Path(tempfile.mkdtemp(prefix="rheo-e2e-"))
