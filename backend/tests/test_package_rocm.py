...
class TestPackage:
    ...
    def test_split_and_manifest(self, tmp_path):
        onedir = tmp_path / "rheo-server-rocm"
        _write(onedir / "rheo-server-rocm.exe")
        ...
        server = out / "rheo-server-rocm.tar.gz"
        libs = out / "rocm-libs-rocm7.2-v1.tar.gz"
        assert server.exists()
        assert libs.exists()
        assert (out / "rheo-server-rocm.tar.gz.sha256").exists()
        ...
        assert "rheo-server-rocm.exe" in core_names
