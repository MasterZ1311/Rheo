...
class TestRocmBuildArgs:
    ...
    def test_binary_name(self, captured_args):
        ...
        assert captured_args[idx + 1] == "rheo-server-rocm"
...
@pytest.mark.slow()
@pytest.mark.skipif(
    sys.platform != "win32" or not _HAS_PYINSTALLER,
    reason="ROCm build E2E requires Windows and PyInstaller to be installed",
)
class TestRocmBuildE2E:
    ...
    def test_rocm_binary_compiles_and_runs(self, tmp_path):
        ...
        binary_dir = dist_dir / "rheo-server-rocm"
        binary_exe = binary_dir / "rheo-server-rocm.exe"
