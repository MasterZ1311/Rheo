...
|----------|------|------------|
| macOS    | `backend/dist/rheo-server-cuda/rheo-server-cuda` | onedir (CUDA, rarely on Mac) |
| macOS    | `backend/dist/rheo-server`                          | onefile (CPU) |
| Windows  | `backend\dist\rheo-server-cuda\rheo-server-cuda.exe` | onedir (CUDA) |
| Windows  | `backend\dist\rheo-server.exe`                      | onefile (CPU) |
...
Mirrors Tauri's launch in `tauri/src-tauri/src/main.rs`:
...
- **Data dir**: `tempfile.mkdtemp(prefix="rheo-e2e-")`. Deleted after the run unless `--keep-data-dir`. Profiles and generated WAVs land here.
...
```
# Rheo E2E — darwin-arm64 — 2026-04-16 12:34
...
- Don't touch the user's HF cache by default — let the server use `HF_HUB_CACHE` / `RHEO_MODELS_DIR`. Passing `--isolated-cache` would point both env vars at the tempdir for a true cold-start run (opt-in only; would re-download every time).
