# ============================================================
# Rheo — Local TTS Server with Web UI
...
RUN groupadd -r rheo && \
    useradd -r -g rheo -m -s /bin/bash rheo
...
COPY --chown=rheo:rheo backend/ /app/backend/

# Copy built frontend from frontend stage
COPY --from=frontend --chown=rheo:rheo /build/web/dist /app/frontend/

# Create data directories owned by non-root user
RUN mkdir -p /app/data/generations /app/data/profiles /app/data/cache \
    && chown -R rheo:rheo /app/data
...
# Entrypoint joins GPU groups then drops to the rheo user.
