...
This feature exposes OpenAI-compatible endpoints from Rheo, allowing any tool, library, or application that speaks the OpenAI Audio API to use Rheo as a drop-in local replacement.
...
- **OpenAI SDK users**: `openai.audio.speech.create()` works with Rheo
- **LLM frameworks**: LangChain, AutoGen, etc. can use Rheo for TTS
...
- `voice` parameter maps to Rheo profile names (case-insensitive)
...
async def resolve_voice_for_openai(voice: str, db: Session) -> Optional[VoiceProfile]:
    """
    Resolve OpenAI voice parameter to a Rheo profile.
