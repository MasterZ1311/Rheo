...
Docker support makes Rheo easier to deploy, especially for:
...
```bash
# CPU-only version
docker run -p 8000:8000 -v rheo-data:/app/data \
  ghcr.io/MasterZ1311/Rheo:latest

# NVIDIA GPU version
docker run --gpus all -p 8000:8000 -v rheo-data:/app/data \
  ghcr.io/MasterZ1311/Rheo:latest-cuda

# AMD GPU version (experimental)
docker run --device=/dev/kfd --device=/dev/dri -p 8000:8000 \
  -v rheo-data:/app/data \
  ghcr.io/MasterZ1311/Rheo:latest-rocm
```
...
services:
  rheo:
    image: ghcr.io/MasterZ1311/Rheo:latest-cuda
    ports:
      - "8000:8000"
    volumes:
      - rheo-data:/app/data
...
```bash
docker build -t rheo .
docker run -p 8000:8000 -v $(pwd)/data:/app/data rheo
```
...
Build:
```bash
docker build -f Dockerfile.optimized -t rheo:slim .
```
...
docker run --device=/dev/kfd --device=/dev/dri \
  ...
  -p 8000:8000 -v rheo-data:/app/data \
  rheo:rocm
```
...
```bash
docker run -v rheo-data:/app/data \           # Profiles, generations, history
...
Configure Rheo via environment variables:
...
doctl compute droplet create rheo \
...
sh get-docker.sh
docker run --gpus all -d -p 80:8000 rheo:cuda
```
...
```bash
# Build and push
docker build -t gcr.io/your-project/rheo .
docker push gcr.io/your-project/rheo

# Deploy to Cloud Run
gcloud run deploy rheo \
  --image gcr.io/your-project/rheo \
  --platform managed \
  --region us-central1 \
  --memory 4Gi \
  --cpu 2 \
...
[build]
  image = "ghcr.io/MasterZ1311/Rheo:latest"
...
```bash
# Fix permissions
docker run --user $(id -u):$(id -g) -v $(pwd)/data:/app/data rheo
```
...
```bash
docker run -e GPU_MEMORY_FRACTION=0.5 rheo
```

Or use CPU-only:
```bash
docker run -e DEVICE=cpu rheo
```
...
```bash
docker run -v huggingface-cache:/root/.cache/huggingface rheo
```

Or use host cache:
```bash
docker run -v ~/.cache/huggingface:/root/.cache/huggingface rheo
```
...
Change host port:
```bash
docker run -p 8080:8000 rheo  # Use port 8080 instead
```
...
docker run -e GPU_MEMORY_FRACTION=0.8 rheo

# Allow GPU memory growth (prevents OOM)
docker run -e TF_FORCE_GPU_ALLOW_GROWTH=true rheo
```
...
4. Report issues: [GitHub Issues](https://github.com/MasterZ1311/Rheo/issues)
