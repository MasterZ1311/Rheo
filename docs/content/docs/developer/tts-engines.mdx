---
title: "TTS Engines"
description: "How to add new text-to-speech engines to Rheo"
---
...
**Do not start writing code until you complete Phase 0.** Bundling issues compound rapidly if dependency research is skipped. Every issue — `inspect.getsource()` failures, missing native data files, metadata lookups, dtype mismatches — is discoverable by reading the model library's source code before integration begins.
...
This is where most of the pain lives. **Bundling requires rigorous validation** — newly added engines (LuxTTS, Chatterbox, Chatterbox Turbo) may work in dev but fail in production builds if PyInstaller directives are missing. Don't skip this phase.
...
### 5.2 Production Bundling Failures and Fixes

These are actual production failures encountered when shipping engines. Every one of these passed `python -m uvicorn` in dev:
...
- **`rheo-server`** (CPU) — excludes all `nvidia.*` packages to avoid bundling ~3 GB of CUDA DLLs
- **`rheo-server-cuda`** — includes `torch.cuda` and `torch.backends.cudnn`
...
You can't skip this. Models that work in `python -m uvicorn` will break in the PyInstaller binary if runtime assumptions fail. Rigorous local testing is required before tagging a release:
...
The [`docs/PROJECT_STATUS.md`](https://github.com/MasterZ1311/Rheo/blob/MZ-Main/docs/PROJECT_STATUS.md) file is the canonical, living list of candidates under evaluation — including why some have been backlogged (e.g. VoxCPM, which is effectively CUDA-only upstream).
