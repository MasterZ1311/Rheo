---
title: "Building"
description: "How Rheo is built for production"
---

## Overview

Rheo uses a two-stage build process:
...
cp dist/rheo-server ../tauri/src-tauri/binaries/rheo-server-${PLATFORM}
```
...
```json
{
  "tauri": {
    "bundle": {
      "externalBin": ["binaries/rheo-server"]
    }
  }
}
```

Tauri looks for `rheo-server-${PLATFORM}` in `src-tauri/binaries/` and bundles it.
...
3. Package with `scripts/package_cuda.py` into two archives:
   - `rheo-server-cuda.tar.gz` — server core (~945 MB)
...
<AccordionGroup>
  ...
  <Accordion title="Tauri can't find sidecar">
    Ensure binary exists at `tauri/src-tauri/binaries/rheo-server-${PLATFORM}` before running Tauri build.
  </Accordion>
</AccordionGroup>
