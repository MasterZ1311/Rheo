---
title: "Auto-Updater"
description: "How Rheo automatic updates work"
---

## Overview

Rheo uses Tauri's built-in auto-updater to deliver signed updates to users. The system verifies updates cryptographically before installation.

## How It Works

When Rheo launches (in production Tauri builds only), it checks GitHub Releases for a `latest.json` manifest. If a newer version is available:
...
```json
{
  "plugins": {
    "updater": {
      ...
      "dialog": false,
      "endpoints": [
        "https://github.com/MasterZ1311/Rheo/releases/latest/download/latest.json"
      ],
...
```json
{
  ...
  "platforms": {
    "darwin-aarch64": {
      ...
      "url": "https://github.com/MasterZ1311/Rheo/releases/download/v0.1.0/rheo_0.1.0_aarch64.app.tar.gz"
    },
    "darwin-x86_64": {
      "signature": "base64_encoded_signature",
      "url": "https://github.com/MasterZ1311/Rheo/releases/download/v0.1.0/rheo_0.1.0_x64.app.tar.gz"
    },
    "linux-x86_64": {
      "signature": "base64_encoded_signature",
      "url": "https://github.com/MasterZ1311/Rheo/releases/download/v0.1.0/rheo_0.1.0_amd64.AppImage"
    },
    "windows-x86_64": {
      "signature": "base64_encoded_signature",
      "url": "https://github.com/MasterZ1311/Rheo/releases/download/v0.1.0/rheo_0.1.0_x64_en-US.msi"
    }
  }
}
```
...
1. **Generate keys** (one-time setup):
   ```bash
   bun tauri signer generate -w ~/.tauri/rheo.key
   ```
   This creates:
   - Private key: `~/.tauri/rheo.key` (stored in GitHub Secrets, never committed)
   - Public key: `~/.tauri/rheo.key.pub` (pasted into `tauri.conf.json`)
...
- `TAURI_SIGNING_PRIVATE_KEY` - Content of `~/.tauri/rheo.key`
...
- CUDA server core: ~945 MB (versioned with each Rheo release)
- CUDA libs (NVIDIA runtime DLLs): ~1.7 GB (versioned independently, cached across upgrades)

### Two-archive split

The CUDA binary is packaged as **two archives** instead of one:

- **Server core** (`rheo-server-cuda.tar.gz`) — the Python server + PyTorch code, changes every release.
...
This means most Rheo upgrades only re-download the ~945 MB server core, not the full ~2.5 GB bundle.
...
5. **Restart** — The Rheo server restarts and swaps in the CUDA backend.
...
{data_dir}/backends/cuda/
  rheo-server-cuda.exe    # Windows
  rheo-server-cuda        # macOS/Linux
...
event: progress
data: {"current": 52428800, "total": 945000000, "filename": "rheo-server-cuda.tar.gz", "status": "downloading"}
```
...
- `rheo-server-cuda.tar.gz` — server-core archive
- `rheo-server-cuda.tar.gz.sha256` — checksum
