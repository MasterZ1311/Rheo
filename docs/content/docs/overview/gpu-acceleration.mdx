...
<Callout type="info">
  The split-archive design (added in v0.4) means most Rheo upgrades only redownload the small server-core archive. The 4 GB libs archive is only refreshed when the underlying CUDA toolkit or torch major version changes.
</Callout>
...
Rheo 0.4 added explicit RTX 50-series support:
...
1. Make sure you're on Rheo **≥ 0.4.0** (Settings → About)
...
<AccordionGroup>
  ...
  <Accordion title="'no kernel image is available' / 'CUDA error'">
    ...
    1. Update to Rheo ≥ 0.4.0 (Blackwell support added there)
