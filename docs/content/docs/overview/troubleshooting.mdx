...
description: "Common issues and solutions for Rheo"
---

This guide covers common issues you might encounter when using or developing Rheo, along with solutions.
...
xattr -cr /Applications/Rheo.app
```
...
<AccordionGroup>
  ...
  <Accordion title="Permission Issues">
    ...
    ```bash
    # macOS/Linux
    chmod +x ~/Library/Application\ Support/sh.rheo.app/backend/rheo-server
    ```
  </Accordion>
  ...
  <Accordion title="Check Logs">
    ...
    **macOS:**
    ```bash
    tail -f ~/Library/Application\ Support/sh.rheo.app/logs/server.log
    ```

    **Windows:**
    ```bash
    type %APPDATA%\sh.rheo.app\logs\server.log
    ```
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Install flash-attn on Windows (community wheels)">
    ...
    Alternatively, run Rheo's backend inside WSL2 and use the standard Linux wheels.
  </Accordion>
</AccordionGroup>
...
**Solutions:**
- Close all Rheo instances
- Delete the lock file:
  ```bash
  # macOS
  rm ~/Library/Application\ Support/sh.rheo.app/data/rheo.db-shm
  rm ~/Library/Application\ Support/sh.rheo.app/data/rheo.db-wal
  ```
...
```bash
# macOS
rm ~/Library/Application\ Support/sh.rheo.app/data/rheo.db

# Windows
del %APPDATA%\sh.rheo.app\data\rheo.db
```
...
<AccordionGroup>
  ...
  <Accordion title="Check GPU Selection">
    If you have multiple GPUs, ensure Rheo is using the right one.
...
1. **Check GitHub Issues:** [github.com/MasterZ1311/Rheo/issues](https://github.com/MasterZ1311/Rheo/issues)
2. **Open a New Issue:** Provide:
   - Operating system and version
   - Rheo version
   - Steps to reproduce
   - Error messages or logs
3. **GitHub Discussions:** [github.com/MasterZ1311/Rheo/discussions](https://github.com/MasterZ1311/Rheo/discussions)
