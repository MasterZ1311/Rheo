...
<Callout type="info">
  The local LLM doubles as the refinement model for [Dictation](/overview/dictation) —
  sharing one model cache and one GPU-memory footprint across the app.
</Callout>
...
The model runs through the same backend split Rheo already uses for TTS
...
<Steps>
  ...
  <Step title="Speak in character — toggle persona rewrite">
    ...
    generate — Rheo runs the text through the personality LLM first,
...
- **Agents that speak in a voice you own.** Combine the persona toggle with
  ...
  personality. The agent calls `rheo.speak({ text, profile, personality:
  true })` and Rheo rewrites the text in character before speaking.
...
`rheo.speak` tool uses when you pass `personality: true`. Scripts and
