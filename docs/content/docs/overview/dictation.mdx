...
a chord, talk, release — Rheo transcribes what you said with Whisper,
optionally cleans it up with a local LLM, and pastes the result into the text
field you had focused when you started.

Everything runs locally on your hardware. Dictation is designed to be invisible: it runs in the background, waits for
the chord, pastes the text, and leaves you where you were.
...
<Steps>
  ...
  <Step title="Text lands in your app">
    If auto-paste is enabled and Rheo has Accessibility permission, the
...
Rheo ships two chord behaviors out of the box:
...
- **Left vs right modifier badges.** When you hold keys into the chord
  picker, Rheo records whether each modifier is the left or right variant.
...
Once transcription finishes, Rheo can synthesize a native paste into
...
- Whisper loop hallucinations — Rheo strips repeated tokens (six or
...
- **UAC / UIPI caveat.** Synthetic paste into an *elevated* window from a
  non-elevated Rheo is blocked by Windows itself. Run Rheo elevated
...
- **Not yet in this release.** The Rust shim ships the macOS and Windows
  paths in this release. Linux `uinput` / AT-SPI support and the Wayland paste
  story are tracked in `docs/plans/VOICE_IO.md`.

## When auto-paste skips itself

A few cases where Rheo deliberately does *not* synthesize a paste:

- **Focus was inside Rheo** when the chord started. The transcript goes
  to the Captures tab so a dictation-into-Rheo round-trip doesn't
