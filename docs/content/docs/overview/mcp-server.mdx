...
Rheo ships a built-in **Model Context Protocol** server so local AI
agents can call your Rheo install directly: speak text in a voice
profile, transcribe audio, and list captures or profiles. The server runs
inside the same process as the rest of Rheo and is mounted at `/mcp`
over Streamable HTTP.

Agent asks to speak → Rheo plays audio on your speakers → an on-screen
pill surfaces the voice name for the whole duration so you always see what's
coming out of your machine.

<Callout type="info">
  The design goal of Rheo MCP is "a local voice layer for every agent on your machine" —
  the same app that captures your voice can generate a response in any voice profile you've
  cloned.
</Callout>
...
Rheo → Settings → MCP. The value is just an identifier for the
per-client voice binding — not a secret, not a credential.

### Clients that only speak stdio

A stdio shim binary `rheo-mcp` is bundled with the desktop app. Point
...
<Tabs items={["macOS", "Windows", "Linux"]}>
  ...
  ```json
  {
    "mcpServers": {
      "rheo": {
        "command": "C:\\Program Files\\Rheo\\rheo-mcp.exe",
        "env": { "RHEO_CLIENT_ID": "claude-desktop" }
      }
    }
  }
...
The shim waits up to 30 seconds for the Rheo backend to come up, then
proxies JSON-RPC from stdio over Streamable HTTP. Rheo must be running
...
| Tool | Use |
|---|---|
| `rheo.speak` | Speak text in a voice profile. Returns a `generation_id` to poll. |
| `rheo.transcribe` | Whisper transcription of base64 audio or an absolute local path. |
| `rheo.list_captures` | Recent captures with transcripts, paginated. |
...
Every call to `rheo.speak` (and `POST /speak`) resolves the voice profile
in this order:
...
<Steps>
  ...
  <Step title="Per-client binding">
    Looked up by the `X-Rheo-Client-Id` header. Managed in
    **Rheo → Settings → MCP**. Lets you pin Claude Code to Morgan,
...
Rheo → Settings → MCP shows one row per `client_id` Rheo has heard
...
| `default_personality` | When true, `rheo.speak` routes through the profile's personality LLM (rewrite) by default. |
...
silent background TTS is a trust hazard, so Rheo always shows what's
...
Start with `rheo.list_profiles` to confirm wiring, then
`rheo.speak` for end-to-end — you should hear audio and see the
generation land in the Captures tab.

<Callout type="info">
  If an agent can't reach the server, the first thing to check is that
  Rheo is running — the backend only listens while the desktop app is
...
- **Localhost only.** The server binds to `127.0.0.1`. If you ever point
  Rheo at a non-loopback interface (e.g. remote-mode over a trusted
  network), add a bearer token — it's on the roadmap for future releases.
- **No auth today.** Any process that can connect to your loopback can
  call MCP. That's the same trust boundary as the rest of Rheo's REST
...
- **Voice cloning consent applies.** See [Voice Cloning](/overview/voice-cloning#limitations)
  — an agent being able to call `rheo.speak` in someone's voice
...
- **Lifespan:** mounting FastMCP requires the `lifespan=` kwarg on
  ...
  with FastMCP's Streamable HTTP session manager. The Rheo app.py
