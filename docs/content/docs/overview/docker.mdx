...
description: "Run Rheo as a headless server with a web UI using Docker"
---

## Overview

Rheo can run as a Docker container with a full web UI -- no desktop app required. This is ideal for headless servers, shared GPU machines, or self-hosted deployments.
...
Open [http://localhost:17493](http://localhost:17493) in your browser. The full Rheo UI is served directly from the backend.
...
services:
  rheo:
    ...
    volumes:
      ...
      - rheo-data:/app/data
      - huggingface-cache:/home/rheo/.cache/huggingface
    environment:
      - LOG_LEVEL=info
    deploy:
      resources:
...
| `RHEO_MODELS_DIR` | (HuggingFace cache) | Custom path for model storage |
| `RHEO_CORS_ORIGINS` | (local origins) | Additional CORS origins, comma-separated |
...
| `rheo-data` | `/app/data` | Profiles, database, cache |
| `huggingface-cache` | `/home/rheo/.cache/huggingface` | Downloaded models (persists across rebuilds) |
...
- **Non-root user** -- the server runs as `rheo`, not `root`
...
For production deployments, put Rheo behind nginx or Caddy with TLS and authentication:

```nginx
server {
    listen 443 ssl;
    server_name rheo.example.com;

    ssl_certificate /etc/ssl/certs/rheo.pem;
    ssl_certificate_key /etc/ssl/private/rheo.key;
...
If you see `{"message": "rheo API", ...}` instead of the web UI, the frontend build may have failed during the Docker build. Check the build logs:
...
docker run -p 17493:17493 ghcr.io/MasterZ1311/Rheo:latest
```
