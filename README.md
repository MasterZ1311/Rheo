<p align="center">
  <img src=".github/assets/icon-dark.webp" alt="Rheo" width="120" height="120" />
</p>

<h1 align="center">Rheo</h1>

<p align="center">
  <strong>The sovereign open-source AI voice studio.</strong><br/>
  Clone any voice. Generate speech. Dictate with zero latency. Talk to AI agents in voices you own.<br/>
  Architected and maintained by <a href="https://github.com/MasterZ1311">MasterZ1311</a> &middot; Licensed under GNU AGPL v3.0.
</p>
...
<p align="center">
  <a href="https://github.com/MasterZ1311/Rheo">GitHub</a> •
...
<p align="center">
  <a href="https://rheo.sh">
    <img src="landing/public/assets/app-screenshot-1.webp" alt="Rheo App Screenshot" width="800" />
  </a>
</p>

<p align="center">
  <em>Click the image above to watch the demo video on <a href="https://rheo.sh">rheo.sh</a></em>
</p>

<br/>

<p align="center">
  <img src="landing/public/assets/app-screenshot-2.webp" alt="Rheo Screenshot 2" width="800" />
</p>

<p align="center">
  <img src="landing/public/assets/app-screenshot-3.webp" alt="Rheo Screenshot 3" width="800" />
</p>
...
Rheo is a sovereign **local-first AI voice studio** — a free and open-source alternative to **ElevenLabs** and **WisprFlow** in one app. Clone voices from a few seconds of audio, generate speech in 23 languages across 7 TTS engines, dictate into any text field with a global hotkey, and give any MCP-aware AI agent a voice of your choosing.

The two cloud incumbents sit on opposite halves of the voice I/O loop — ElevenLabs on output, WisprFlow on input. Rheo does both, bridges them with a bundled local LLM for refinement and per-profile personas, and runs the whole thing on your machine.
...
- **Agent voice output** — one tool call (`rheo.speak`) and any MCP-aware agent (Claude Code, Cursor, Cline) speaks to you in a voice you've cloned
...
| Platform              | Download                                               |
| --------------------- | ------------------------------------------------------ |
| macOS (Apple Silicon) | [Download DMG](https://rheo.sh/download/mac-arm)   |
| macOS (Intel)         | [Download DMG](https://rheo.sh/download/mac-intel) |
| Windows               | [Download MSI](https://rheo.sh/download/windows)   |
| Docker                | `docker compose up`                                    |

> **[View all binaries →](https://github.com/MasterZ1311/Rheo/releases/latest)**

> **Linux** — Pre-built binaries are not yet available. See [rheo.sh/linux-install](https://rheo.sh/linux-install) for build-from-source instructions.
...
The other half of the voice I/O loop. Hold a hotkey anywhere on your system, speak, release — on macOS the transcript pastes straight into the focused text field. Or hit the mic on any Rheo text input and dictate directly into the app.
...
- **In-app mic button** on every Rheo text field — generation form, profile descriptions, story titles, anywhere you'd type
...
Rheo runs OpenAI Whisper for transcription — the same model that backs dictation, the Captures tab, and the `/transcribe` API. Running on MLX (Apple Silicon) or PyTorch (CUDA / ROCm / DirectML / CPU) depending on your platform.
...
- **Local capture storage** — original audio and transcript stay in your Rheo data directory, with a folder shortcut in Settings
...
- **HTTP + stdio transports** — install as a URL in Claude Code / Cursor / Windsurf / VS Code MCP, or point stdio-only clients at the bundled `rheo-mcp` binary
...
Agents can reach the same rewrite path over MCP by passing `personality: true` to `rheo.speak`, turning the tool into a text-in → personality-LLM → TTS pipeline. The same LLM backs dictation's refinement step — one LLM in the app, one model cache, one GPU-memory footprint.
...
- Custom models directory via `RHEO_MODELS_DIR`
...
Rheo exposes a REST API for integrating voice I/O into your own apps and agents.
...
Rheo ships a built-in **Model Context Protocol** server so any MCP-aware agent (Claude Code, Cursor, Windsurf, Cline, VS Code MCP extensions) can speak, transcribe, and browse captures and profiles.
...
```json
{
  "mcpServers": {
    "rheo": {
      "url": "http://127.0.0.1:17493/mcp",
      "headers": { "X-Rheo-Client-Id": "cursor" }
    }
  }
}
```

**Stdio fallback** for clients that don't speak HTTP MCP — point at the bundled `rheo-mcp` binary inside the app:
...
```json
{
  "mcpServers": {
    "rheo": {
      ...
      "env": { "RHEO_CLIENT_ID": "claude-desktop" }
    }
  }
}
```

Four tools ship: `rheo.speak`, `rheo.transcribe`, `rheo.list_captures`, `rheo.list_profiles`. Per-client voice bindings are managed in **Rheo → Settings → MCP**. See the [full MCP guide](docs/content/docs/overview/mcp-server.mdx) for tool signatures, resolution precedence, the speaking-pill contract, and security notes.
...
For complete endpoint specifications, schemas, and curl examples, see the [API Reference Guide](docs/API.md) and the [OpenAPI Specification](docs/openapi.json). Interactive Swagger UI is available at `http://127.0.0.1:17493/docs` and ReDoc at `http://127.0.0.1:17493/redoc`.
...
| **Mobile companion**               | Control Rheo from your phone                                         |
...
#### Option A: One-Click Setup with `just` (Recommended)

```bash
git clone https://github.com/MasterZ1311/Rheo.git
cd Rheo

# Prerequisites: Install Bun (bun.sh), Python 3.11+, Rust (rustup.rs), and just (brew install just / cargo install just)
just setup   # creates Python venv, installs all backend and frontend dependencies
just dev     # starts the backend server + desktop application concurrently
```

#### Option B: Manual Setup (Without `just`)
...
3. **Set up and start the web frontend (in a separate terminal):**
   ```bash
   bun install
   cd app && bun run dev
   ```
...
```
Once running, open `http://127.0.0.1:17493` in your browser.

The repository includes a pre-configured `.mcp.json` at the root — running Claude Code inside this folder automatically connects to Rheo's MCP server while the app is active.
...
<p align="center">
  <a href="https://rheo.sh">rheo.sh</a>
</p>
