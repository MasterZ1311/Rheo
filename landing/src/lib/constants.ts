...
// $RHEO — community ecosystem token
export const TOKEN_TICKER = '$RHEO';
...
export const TOKEN_DEV_WALLETS: string[] = envList('TOKEN_DEV_WALLETS', [
  'BSn573bjkQa5iffMg6zA8eb9mHyzewu2ps9R85qNKXC5', // MasterZ1311 dev/treasury wallet
]);

/**
 * Locked supply: token accounts whose $RHEO is locked (liquidity lockers,
...
export interface LockedAccount {
  label: string;
  /** The token account or owner address holding the locked $RHEO. */
  account: string;
...
export const TOKEN_PROOFS: TokenProof[] = [
  {
    ...
    txUrl: '', // TODO(MasterZ1311): add the Solscan/locker link for the launch lock
  },
  {
    kind: 'burn',
    label: 'Buyback & burn',
    detail:
      'Bought $RHEO back from the open market and burned it to a dead address, permanently removing it from supply.',
    txUrl:
  ...
  },
  {
    ...
    txUrl: '', // TODO(MasterZ1311): add the Solscan link for the second burn
  },
];
