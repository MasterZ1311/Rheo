...
setup-python:
    ...
    if [ "$(uname)" = "Linux" ]; then
        ...
        if [ -e /proc/driver/nvidia/version ] || [ -d /sys/module/nvidia ]; then
            ...
            if [ -n "${RHEO_ROCM_VERSION:-}" ]; then
                rocm_ver="$RHEO_ROCM_VERSION"
...
build-server: _ensure-venv
    ...
    Copy-Item "backend/dist/rheo-server.exe" "{{ tauri_dir }}/src-tauri/binaries/rheo-server-$triple.exe" -Force; \
    Write-Host "Copied sidecar: rheo-server-$triple.exe"; \
    & "{{ python }}" backend/build_binary.py --shim; \
    if ($LASTEXITCODE -ne 0) { throw "build_binary.py --shim failed with exit code $LASTEXITCODE" }; \
    Copy-Item "backend/dist/rheo-mcp.exe" "{{ tauri_dir }}/src-tauri/binaries/rheo-mcp-$triple.exe" -Force; \
    Write-Host "Copied sidecar: rheo-mcp-$triple.exe"
...
build-server-cuda: _ensure-venv
    ...
    $dest = "$env:APPDATA/sh.rheo.app/backends/cuda"; \
    if (Test-Path $dest) { Remove-Item -Recurse -Force $dest }; \
    New-Item -ItemType Directory -Path $dest -Force | Out-Null; \
    Copy-Item "backend/dist/rheo-server-cuda/*" $dest -Recurse -Force; \
...
[unix]
db-reset:
    rm -f {{ backend_dir }}/data/rheo.db
    just db-init

[windows]
db-reset:
    if (Test-Path "{{ backend_dir }}/data/rheo.db") { Remove-Item -Force "{{ backend_dir }}/data/rheo.db" }
    just db-init
