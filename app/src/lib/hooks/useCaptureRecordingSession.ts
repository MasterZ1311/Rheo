...
export function useCaptureRecordingSession(
  ...
  const stopRecording = useCallback(() => {
    if (captureSettings?.audio_feedback) {
      playAudioFeedback('stop', captureSettings.sound_theme, captureSettings.audio_feedback_volume);
    }
    stopAudioRecording();
  }, [captureSettings, stopAudioRecording]);
  ...
  const startRecording = useCallback(() => {
    ...
    if (captureSettings?.audio_feedback) {
      playAudioFeedback('start', captureSettings.sound_theme, captureSettings.audio_feedback_volume);
    }
    beginAudioRecording();
  }, [isRecording, beginAudioRecording, clearRestTimer, captureSettings]);
