...
export function DictateWindow() {
  ...
  const session = useCaptureRecordingSession({
    onFinalText: async (text, _capture, allowAutoPaste) => {
      ...
      try {
        await invoke('paste_final_text', {
          text,
          focus,
          pasteDelayMs: captureSettings?.paste_delay_ms ?? 50,
          autoSubmit: captureSettings?.auto_submit ?? false,
          autoSubmitKey: captureSettings?.auto_submit_key ?? 'enter',
        });
      } catch (err) {
