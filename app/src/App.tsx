...
/**
 * Validate that a health response has the expected Rheo-specific shape.
 * Prevents misidentifying an unrelated service on the same port.
 */
function isRheoHealthResponse(health: HealthResponse): boolean {
  return (
...
function MainApp() {
  ...
  useEffect(() => {
    if (!platform.metadata.isTauri) {
      ...
      if (currentServerUrl !== serverUrl && isLoopbackRheoServerUrl(currentServerUrl)) {
    ...
    if (!import.meta.env?.PROD) {
      ...
      window.__rheoServerStartedByApp = false;
      return;
    }
    ...
    platform.lifecycle
      ...
      .then((serverUrl) => {
        ...
        window.__rheoServerStartedByApp = true;
      })
      .catch((error) => {
        ...
        // until it responds with a valid Rheo payload, then transition to
        // the main UI.
        console.log('Falling back to health-check polling...');
        const pollInterval = setInterval(async () => {
          try {
            const health = await apiClient.getHealth();
            if (!isRheoHealthResponse(health)) {
              console.log('Health response is not from a Rheo server, keep polling...');
              return;
            }
            console.log('External Rheo server detected via health check');
        ...
        setTimeout(() => {
          ...
          serverStartingRef.current = false;
          setStartupError(
            'Could not connect to a Rheo server within 2 minutes. ' +
